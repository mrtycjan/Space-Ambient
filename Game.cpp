...
{	
	...
	while (state == OVER)
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (text[12].getGlobalBounds().contains(mouse) && event.type == sf::Event::MouseButtonReleased && event.key.code == sf::Mouse::Left || sf::Keyboard::isKeyPressed(sf::Keyboard::Escape))
				state = ENGINE;
