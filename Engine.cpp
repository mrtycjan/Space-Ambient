...
void Engine::control()
{
    ...
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Space) || sf::Keyboard::isKeyPressed(sf::Keyboard::LControl))
    {
        ...
        {
            playerBullet.add(player.getPosition(), player.getRotation(), 320, 18);
            time = sf::Time::Zero;
        }
    }
}
...
void Engine::gamePlay()
{
    ...
    if(time >= sf::seconds(2))
    {
		...
		{	
			...
			ufoBullet.add(ufo.getPosition(index), ufo.getRotation(index), 150, 28);
		}
    ...
    for(int i=0; i<playerBullet.getAmount(); i++)
    {
        ...
        for(int j=0; j<ufo.getAmount(); j++)
        {
            if(ufo.getRect(j).intersects(playerBullet.getRect(i)))
            {
				...
				{	
					explosion.add(ufo.getSprite(j).getPosition());
					ufo.remover(j);
					score += 30;
				}
...
void Engine::update(float delta)
{
    ...
    ufo.update(player.getPosition(), delta);
	explosion.update();
}
Engine::Engine(sf::RenderWindow &win, sf::Vector2f &win_size,  Audio &sound, int index): id(index)
{	
	score = 0;
    delta = 0;
    bounds.x = 5000;
    ...
    texture.loadFromFile("data/textures/blue.png");
